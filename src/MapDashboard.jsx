...
const MapDashboard = () => {
  ...
  // Auto-select first pincode
  useEffect(() => {
    if (allPins.length > 0 && !filterPin) {
      setFilterPin(String(allPins[0]));
    }
  }, [allPins]);
  ...
  // Filter logic
  const filteredData = data.filter((item) => {
    if (!filterPin) return true; // all pincode mode
    return String(item.pincode) === String(filterPin);
  });

  // Pagination only when All Pincodes
  const paginatedData =
    filterPin === ""
  ...
  return (
    <div>
      ...
      {/* Pagination UI (only when ALL pincodes) */}
      {filterPin === "" && (
        <div
          style={{
            display: "flex",
